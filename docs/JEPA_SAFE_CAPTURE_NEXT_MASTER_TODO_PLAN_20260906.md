...
- **P3 hard-negative archive collection：已通过 collection gate，训练仍暂停。** 在 train split 生成 `1872` 条样本（runtime `1728`、boundary-shadow `144`）；不重叠 validation 生成 `1508` 条（runtime `1392`），calibration 生成 `2652` 条（runtime `2448`）。validation 同时含有正/负 horizon feasibility、boundary、visibility 和 route-geometry 类别；三套 archive 都新增 stopping-distance、obstacle/boundary/pairwise TTC、acceleration-slack 五类有限 `[N,5]` 标签。该结果只通过数据合同 gate，不是 safe-capture 或 JEPA 性能提升证据；下一步先做标签方向/校准审计，再决定 head-only training smoke。详见 [P3 hard-negative archive report](JEPA_SAFE_CAPTURE_P3_HARD_NEGATIVE_ARCHIVE_20260907.md)。
- **P3 label audit：已通过，模型校准仍暂停。** 三套 archive 的五类风险标签均 finite；TTC 在 `[0,10]`、stopping distance 非负、visibility 为二值；failure 与 CBF feasibility 一致；runtime 无负 boundary clearance，负 boundary 只存在明确标记的 offline shadow 行；`-1.0` acceleration slack 被审计为失败/未知 sentinel。该审计只证明标签合同和覆盖可计算，不证明模型预测校准或 safe-capture 提升。详见 [P3 hard-negative archive report](JEPA_SAFE_CAPTURE_P3_HARD_NEGATIVE_ARCHIVE_20260907.md)。
- **P4 head-only smoke：已停止扩大。** 冻结 v1 route JEPA backbone 后新增五个风险头，8 epoch validation loss 下降；但 held-out pairwise TTC `<1s` recall 只有 validation `3.8%`、calibration `3.5%`，boundary/obstacle hazard recall 也不足，不能安全驱动 Ledger 排序。归因是 clipped-TTC 回归和风险尾部不平衡，而不是 CBF 违规；因此未启动 full JEPA、三 seed、Ledger 或闭环。下一步先改为 TTC hazard classification/quantile + censored slack mask，再重复同一 held-out audit。详见 [P4 head-only stop report](JEPA_SAFE_CAPTURE_P4_HEADONLY_SMOKE_STOP_20260907.md)。
- **P4 hazard-head smoke：已停止扩大。** 加入 `<=0.5/1/2s` TTC hazard 分类和 quantile loss 后，validation pairwise recall 提升到 `56.6%/53.4%/68.6%`，但 calibration 只有 `39.3%/37.9%/49.5%`，且 `<1s` 在 cutoff `0.2` 时虽达 `84.8%` recall，precision 仅 `31.8%`；直接接入 Ledger 会再次造成过度 safe-hold。结论是 prediction signal improved but pairwise tail generalization 未通过，停止 full JEPA、三 seed、Ledger 和闭环。下一步只做 pairwise-specific representation/data audit。详见 [P4 hazard-head report](JEPA_SAFE_CAPTURE_P4_HAZARD_HEAD_SMOKE_20260907.md)。
- **P4 pairwise interaction-tail audit + pooling smoke：已完成并停止扩大。** 从 63-D 公共观测重建 relative teammate position/velocity 与 formation topology；calibration 的直接 observation pairwise TTC proxy 对 `<=1s` 风险达到 AUC `0.796`，说明交互信息可观测。随后只训练 pairwise pooling 与风险头，calibration 默认 cutoff `0.5` 为 `42.9% recall / 40.1% precision`，相较旧 head-only 的 `37.9% / 37.8%` 仅小幅改善，仍未达到 `recall>=80% 且 precision>=50%` 的准入门槛。结论是 interaction representation/calibration 仍不足，停止 full JEPA、new Ledger 和闭环；详见 [pairwise audit](JEPA_SAFE_CAPTURE_P4_PAIRWISE_INTERACTION_TAIL_AUDIT_20260907_v2.md) 与 [pooling stop report](JEPA_SAFE_CAPTURE_P4_PAIRWISE_POOLING_SMOKE_STOP_20260907.md)。
...
- [x] 在每个 episode/step 记录：`requested_action`、`projected_action`、`cbf_action`、`executed_action`、最小 slack、最早失败 step、失败约束类型和 fallback reason；新 trace schema smoke 见 [P0 trace-schema smoke](JEPA_SAFE_CAPTURE_P0_TRACE_SCHEMA_SMOKE_20260907.md)。
...
- [x] 生成 train-only 含 hard negative 的 counterfactual archive smoke；该结果不是 safe-capture 提升证据，详见 [P3 hard-negative archive report](JEPA_SAFE_CAPTURE_P3_HARD_NEGATIVE_ARCHIVE_20260907.md)。
- [x] 生成与 train seed 不重叠、同时含正负 feasibility/boundary/visibility 的 validation archive，并完成 calibration archive smoke；P3 collection gate 已通过，标签校准审计仍未完成。
...
**下一步唯一动作：** 若继续推进，只允许一个新的 bounded offline 设计迭代：将 pairwise pooling 扩展为候选动作条件下的相对速度投影、formation topology 和 per-pair risk，并在全新 disjoint calibration archive 上复测同一门槛；在 calibration pairwise hazard `recall>=80%` 且 `precision>=50%` 达标前，不训练 full JEPA、不构建新 Ledger、不运行闭环，也不扩大数据量。
