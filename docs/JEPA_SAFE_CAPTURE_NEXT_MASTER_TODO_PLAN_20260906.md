...
**下一步唯一动作：** 先重设计 TTC hazard classification/quantile 和 acceleration-slack censored mask，使用同一 train/validation/calibration archive 做小 smoke；在 pairwise/boundary high-risk recall 达标前不训练 full JEPA、不构建新 Ledger、不运行闭环。
