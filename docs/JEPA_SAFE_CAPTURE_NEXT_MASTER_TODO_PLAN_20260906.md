...
**下一步唯一动作：** 基于已通过的 archive 标签审计，先做多任务 JEPA head-only 训练 smoke，并在 validation 上检查预测方向、finite rate、Brier/ECE 和 hard-slice recall；若 paired safe-capture 没有改善，立即停止模型/数据扩展并回到 route/CBF/Ledger 归因。
