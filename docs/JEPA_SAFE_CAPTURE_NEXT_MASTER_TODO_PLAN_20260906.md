...
- **P3 hard-negative archive train smoke：已完成但未通过完整 P3 gate。** 在 train split、冻结 actor、`horizon=5 + strict_buffer` 合同下生成 `1872` 条样本（runtime `1728`、boundary-shadow `144`）；首步 CBF 可行率 `100%`，horizon 内失败 `572/1728`，并新增 stopping-distance、obstacle/boundary/pairwise TTC、acceleration-slack 五类 `[N,5]` 有限标签。该结果证明后续 rollout 负例存在，但只覆盖 train smoke，尚无 held-out validation/calibration，因此停止 JEPA 训练，先生成不重叠 validation archive。详见 [P3 hard-negative archive report](JEPA_SAFE_CAPTURE_P3_HARD_NEGATIVE_ARCHIVE_20260907.md)。
...
- [x] 生成 train-only 含 hard negative 的 counterfactual archive smoke；该结果不是 P3 完成，也不是 safe-capture 提升证据，详见 [P3 hard-negative archive report](JEPA_SAFE_CAPTURE_P3_HARD_NEGATIVE_ARCHIVE_20260907.md)。
- [ ] 生成与 train seed 不重叠、同时含正负 feasibility/boundary/visibility 的 validation archive，并完成 P3 gate。
...
**下一步唯一动作：** 先生成不与 train 重叠的 validation/calibration archive，并检查五类风险标签的方向、覆盖和校准可计算性；在 P3 gate 通过前不启动新 JEPA 训练，也不扩大 L1–L3 或多 seed。
