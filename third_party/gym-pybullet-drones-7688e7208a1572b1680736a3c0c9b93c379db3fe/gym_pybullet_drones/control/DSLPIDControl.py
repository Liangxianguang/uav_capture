...
def _matrix_to_xyz_euler(rotation: np.ndarray) -> np.ndarray:
    """Convert a proper rotation matrix to intrinsic XYZ Euler angles."""
    value = float(np.clip(rotation[0, 2], -1.0, 1.0))
...
class DSLPIDControl(BaseControl):
    ...
    def _dslPIDPositionControl(self,
        ...
        scalar_thrust = max(0., float(np.sum(target_thrust * cur_rotation[:, 2])))
        ...
        #### Target rotation #######################################
        target_euler = _matrix_to_xyz_euler(target_rotation)
    ...
    def _dslPIDAttitudeControl(self,
        ...
        target_quat = np.asarray(p.getQuaternionFromEuler(target_euler), dtype=np.float64)
        target_rotation = np.asarray(p.getMatrixFromQuaternion(target_quat), dtype=np.float64).reshape(3, 3)
        first_product = np.sum(target_rotation.transpose()[:, None, :] * cur_rotation[None, :, :], axis=2)
        second_product = np.sum(cur_rotation.transpose()[:, None, :] * target_rotation[None, :, :], axis=2)
        rot_matrix_e = first_product - second_product
        ...
        pwm = thrust + np.sum(self.MIXER_MATRIX * target_torques[None, :], axis=1)
